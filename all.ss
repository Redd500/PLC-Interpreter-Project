         ...
         [(equal? 'let* (1st datum))
          (letrec ((create-let-exp
                     (lambda (exp)
                       (if (null? exp)
                           (map parse-exp (cddr datum))
                           (let-exp (list (list (caar exp)
                                      (parse-exp (cadar exp))))
                             (if (null? (cdr exp))
                                 (create-let-exp (cdr exp))
                                 (list (create-let-exp (cdr exp)))))))))
    ...
    (cases expression exp
      [let-exp (arguments bodies)
        (app-exp (lambda-exp (map 1st arguments) (map syntax-expand bodies))
          (map syntax-expand (map 2nd arguments)))]
